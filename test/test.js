...
test("testing for delete", (t)=> {
    ...
    const actual1 = todoFunctions.deleteTodo(stateBeforeTest, -1);
    const newState = [
        { id: -3, description: 'first todo', done: false},
        { id: -2, description: 'second todo', done: false}
    ]
    let newStatePure = todoFunctions.cloneArrayOfObjects(newState);

    const expected2 = [ 
        { id: -3, description: 'first todo', done: false}
    ];
    const actual2 = todoFunctions.deleteTodo(newStatePure, -2);
    ...
    t.deepEqual(newState, newStatePure,"delete function must be pure");
    t.end();
});
...
test("Test of the state array with addTodo function" , (t)=> {
    ...
    t.deepEqual(todoFunctions.addTodo(stateBeforeTest , 'how are you') , expected1 , "first add test");
    t.deepEqual(state, stateBeforeTest,"add function didnt change on the input(pure function)");
    t.deepEqual(todoFunctions.addTodo(stateBeforeTest , 'thank you') , expected2 , "second add test");
    t.deepEqual(todoFunctions.addTodo(stateBeforeTest , 'you are welcome') , expected3 , "third add test");
...
test("testing for markTodo",(t)=>{
    ...
    t.deepEqual(todoFunctions.markTodo(stateBeforeTest, -1),expected1,"marking id -1 test");
    t.deepEqual(state, stateBeforeTest,"mark function didnt change on the input(pure function)");
    t.deepEqual(todoFunctions.markTodo(stateBeforeTest, -2),expected2,"marking id -2 test");
    t.deepEqual(todoFunctions.markTodo(stateBeforeTest, -3),expected3,"marking id -3 test");
    t.end();
})
...
test("testing for sort function", t=>{
    ...
    const actual1 = todoFunctions.sortTodos(unSort1Pure,todoFunctions.sortFunction);
